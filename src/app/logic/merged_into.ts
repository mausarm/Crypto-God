...
import { STATUS } from '../store/global_constants';
...
export function mergedInto(mergeAssets: ReadonlyArray<Asset>, intoAssets: ReadonlyArray<Asset>): ReadonlyArray<Asset> {
    const returnAssets: Asset[] = intoAssets.map((ia) => {
        ...
        if (ma) {
            return new Asset(ia.id, ma.symbol, ma.name, ia.status, ma.logo_url, ma.price, ma.first_trade, ma.history, ia.amount_history, ia.amount_timestamps);
        }
        else {
            return ia.clone();
        }
